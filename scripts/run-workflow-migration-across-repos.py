...
def run_script(
    script_path: Path, repo_path: Path, extra_args: List[str] = None
) -> Dict[str, Any]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run workflow migration across repositories"
    )
    parser.add_argument(
        "--workspace-root",
        type=Path,
    ...
    # Define script paths
    remove_permissions_script = (
        scripts_repo / "scripts" / "remove-reusable-workflow-permissions.py"
    )
    ...
    for repo_name, repo_results in results.items():
        ...
        validate_success = repo_results.get("validation", {}).get(
            "success", True
        )  # Default True for dry-run
        ...
        if remove_success and update_success and validate_success:
            ...
            # Extract file counts
            remove_summary = parse_script_output(
                repo_results["remove_permissions"].get("stdout", "")
            )
            update_summary = parse_script_output(
                repo_results["update_calling"].get("stdout", "")
            )

            modified = remove_summary.get("modified", 0) + update_summary.get(
                "modified", 0
            )
            processed = remove_summary.get("files_processed", 0) + update_summary.get(
                "files_processed", 0
            )
